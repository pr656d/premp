---
name: reflect-premp
description: Reflect Lovable's main branch from pr656d/prem-s-dot-notebook into the notebook-preview branch of pr656d/premp for Netlify branch deploy
---
...
- From: `git@github.com:pr656d/prem-s-dot-notebook.git`, branch `main` — where Lovable ("Prem's Dot Notebook" project) commits.
...
1. Lovable project "Prem's Dot Notebook" connected to GitHub as `pr656d/prem-s-dot-notebook` (Lovable editor → GitHub → Connect; done by Prem in the Lovable UI).
2. Remote configured in this repo:
   ```bash
   git remote add lovable-source git@github.com:pr656d/prem-s-dot-notebook.git
   ```
...
git push origin refs/remotes/lovable-source/main:refs/heads/notebook-preview --force
```
