...
- `AGENTS.md` (repo root) — Lovable's own stack instructions for the app code + project config
- `about-me.md`, `resume/` — resume system sources
- `.agents/` — agent-agnostic config source of truth (`skills/`, `mcp_config.json`)

## Commands
- Local dev: `bun install && bun run dev`
- Build: `bun run build`
- Regenerate per-CLI configs: `~/ai-config/skills/agent-agnostic-config/scripts/project-setup.sh ~/Documents/Projects/premp`
